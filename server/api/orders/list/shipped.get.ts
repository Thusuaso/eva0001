...
export default defineEventHandler(async()=>{
    const shippedSql = `
        select 
            ...
            s.ID as SiparisId,
        s.SiparisNo,
        TRIM(STR(DAY(s.YuklemeTarihi))) + '-' + TRIM(STR(MONTH(s.YuklemeTarihi))) + '-' + TRIM(STR(YEAR(s.YuklemeTarihi)))  as YuklemeTarihi,
        s.OdemeTurID,
        ot.OdemeTur,
        s.TeslimTurID,
        stt.TeslimTur,
    ...
    return new Promise(async (resolve,reject)=>{
        await mssql.query(shippedSql,(err:any,shipped:any)=>{
            if(err){
                ...
                resolve({'error':false,'shipped':shipped?.recordset});
            }
        });
    });
});