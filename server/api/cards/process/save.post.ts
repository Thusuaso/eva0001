...
export default defineEventHandler(async(event)=>{
    ...
    const insertCardSql = `
        ...
        VALUES('${productId}','${surfaceId}','${sizeId}','${categoryId}')
    `;
    const getCardIdSql = `select top 1 uk.ID  from UrunKartTB uk order by uk.ID desc`;
    return new Promise(async (resolve,reject)=>{
        await mssql.query(insertCardSql,async (err:any,card:any)=>{
            if(err){
                resolve({'error':true});
            }else{  
                if(card.rowsAffected[0]==1){
                    await mssql.query(getCardIdSql,async (err:any,id:any)=>{
                        if(id.recordset.length >0){
                            resolve({'error':false,'status':true,'id':id.recordset[0].ID,'sizeID':sizeId});
                        }
                    });
                }else{
